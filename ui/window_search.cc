    ...
    m_freeSlots(false)
{
    ...
    m_type = display::TYPE_SEARCHWINDOW;
...
SearchResult *WindowSearch::get_result()
{
    ...
    for(unsigned int i=0; i<m_results.size(); ++i) {
        result = m_results[i];
        if(cid == result->getUser()->getCID().toBase32() &&
...
void WindowSearch::search(const std::string &str)
{
    ...
    */
    ...
    m_lastSearch = TimerManager::getInstance()->getTick();
    SearchManager::getInstance()->search(m_search, 0, SearchManager::TYPE_ANY, SearchManager::SIZE_DONTCARE, "");
}
...
void WindowSearch::on(SearchManagerListener::SR, SearchResult* result)
    throw()
{
    try {
        result->incRef();
...
void WindowSearch::add_result(SearchResult *result)
{
    if(matches(result)) {
        int row = insert_row();
        set_text(0, row, result->getUser()->getCID().toBase32() + "-" + result->getTTH().toBase32());
...
bool WindowSearch::matches(SearchResult *result)
{
...
void WindowSearch::download(const std::string &path)
{
    ...
    try {
        if(result->getType() == SearchResult::TYPE_FILE) {
            ...
            std::string subdir = Util::getFileName(utils::linux_separator(result->getFile()));
            ...
            QueueManager::getInstance()->addDirectory(result->getFile(),
                result->getUser(), target, QueueItem::DEFAULT);
        }
    }
    catch(const Exception &e)
    {
...
void WindowSearch::download_directory(const std::string &path)
{
    ...
    try {
        if(result->getType() == SearchResult::TYPE_FILE)
        {
            QueueManager::getInstance()->addDirectory(utils::windows_separator(Util::getFilePath (utils::linux_separator(result->getFile ()))),
                result->getUser(),
                target.empty() ? SETTING (DOWNLOAD_DIRECTORY) : target + "/",
                QueueItem::DEFAULT);
        }
        else
        {
            QueueManager::getInstance()->addDirectory(result->getFile(),
                result->getUser(), target);
        }
    }
    catch(Exception &e)
    {
...
std::string WindowSearch::get_infobox_line(unsigned int n)
{
    ...
    switch(n) {
        ...
        case 2:
            ...
            break;
        case 3:
            ss << "%21Hub:%21 " << result->getHubURL() << " " << result->getHubName();
            break;
        case 4:
        {
