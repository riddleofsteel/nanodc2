...
void WindowHubs::show_entries()
{
    ...
    std::vector<HubEntry>::iterator it = m_hubs.begin();
    for(;it != m_hubs.end() && !m_shutdown; ++it) {
        if(matches(*it)) {
            int row = insert_row();
            set_text(0, row, it->getUsers());
            set_text(1, row, Util::formatBytes(it->getMinShare()));
            set_text(2, row, it->getServer());
            set_text(3, row, it->getName());
        }
    }
...
bool WindowHubs::matches(HubEntry &entry)
{
...
void WindowHubs::on(DownloadFinished, const std::string &list)
    throw()
{
    utils::Lock l(m_mutex);
