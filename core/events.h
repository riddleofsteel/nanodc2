...
/** Event priorities. */
enum Priority {
    FIRST = boost::signals::at_front,
    DEFAULT = 5,
    LAST = boost::signals::at_back
};
...
public:
    ...
    boost::signals::connection add_listener(const std::string &event,
        EventFunc &func, Priority priority=DEFAULT)
...
inline
boost::signals::connection add_listener(const std::string &event, EventFunc func)
{
    return events::Manager::get()->add_listener(event, func);
}

inline
boost::signals::connection add_listener_last(const std::string &event, EventFunc func)
{
    return events::Manager::get()->add_listener(event, func, LAST);
}

inline
boost::signals::connection add_listener_first(const std::string &event, EventFunc func)
{
